...
2. 分类、平台和使用方式筛选按钮。
3. 8 张工具卡片，显示名称、简述、地区、类别、平台、安装/免安装标签和最后核验日期。
4. 无匹配结果时的可访问空状态与「清除筛选」操作。

筛选在浏览器中基于卡片 `data-*` 属性完成，不发送网络请求。第一版不提供搜索框。
...
在约 1280px 桌面宽度和 390px 移动宽度检查目录页、至少一篇命令行工具教程、一篇桌面工具教程和一篇网页工具教程。检查导航、筛选、复制按钮、图片清晰度、键盘操作、控制台错误和无效链接。
