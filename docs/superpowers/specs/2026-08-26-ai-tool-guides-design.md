...
- 结论：不复用其 `doc-builder` 代码或依赖。当前页面是既有 Astro 站中的 8 项静态目录，直接复用其分组信息架构比引入第二套文档工具更兼容；最终采用左侧模型名称目录与右侧分类卡片区。
...
3. 右侧按同样四类展示 8 张工具卡片，名称目录使用锚点定位；桌面两列、640px 以下单列，卡片显示名称、简述、地区、类别、平台、安装/免安装标签和最后核验日期。
...
- 左侧与右侧均有四个分类分组，8 个模型名称链接准确指向对应卡片，分类和工具数量一致。
...
在约 1280px 桌面宽度和 390px 移动宽度检查目录页、至少一篇命令行工具教程、一篇桌面工具教程和一篇网页工具教程。目录页检查桌面粘性侧栏、四类分组、8 个名称锚点、两列卡片、移动折叠目录与单列卡片；同时检查导航、定位高亮、复制按钮、图片清晰度、键盘操作、控制台错误和无效链接。
...
独立功能分支包含按四类组织的模型名称目录、8 篇完整图文教程、所需图片、锚点定位与复制增强、内容/页面契约测试以及同步后的开发文档。所有 8 篇都标明真实核验日期、官方资料、费用、隐私和开源状态；首页保持不变。完整构建、自动化测试和桌面/移动浏览器验收通过后，才建议合并到 `main`。
