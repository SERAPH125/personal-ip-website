...
状态：已实施；2026-08-26 同步配套工具箱

分支：`codex/learning-paths`

> **配套工具箱更新（2026-08-26）：** 用户确认把新工具指南同步到学习路线。两条路线仍各保留 6 篇文章，并在路线末尾分别增加 6 款精选工具；工具名称、分类、安装摘要和详情链接从 `tools` collection 读取，不复制安装正文。左侧目录增加工具箱锚点，820px 以下沿用折叠目录，640px 以下工具卡单列。
...
2. 桌面端左侧粘性学习目录，按两条路线分组列出全部 12 个步骤；右侧保留主要阅读内容。
3. 820px 以下使用默认折叠的原生学习目录，展开后可定位步骤，选择后自动收起。
4. 两个路线章节，每章按 `learning.step` 渲染 6 篇文章。
5. 每个步骤显示序号、标题、摘要、系列和阅读入口；目录锚点只负责定位，不改变文章 canonical。

布局参考开源 [Hugging Face Hub Docs](https://github.com/huggingface/hub-docs) 的分组侧栏层级。该项目采用 Apache-2.0 许可证且持续维护，适合借鉴文档导航的信息架构；它依赖 Hugging Face `doc-builder`，与当前 Astro 静态站构建链不兼容，因此不复用其代码或依赖，只在现有 Astro 模板与 CSS 中实现同类布局。
