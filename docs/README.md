...
`tools.html` 在桌面端采用 Hugging Face 文档式左侧粘性筛选目录，类别、平台、使用方式三组按钮放在侧栏中，右侧以两列展示工具卡片；820px 以下侧栏改为默认折叠的原生筛选目录，640px 以下卡片单列。组合筛选仍由 `public/assets/hub.js` 完成，命令行文章的代码块渐进增强为可复制。JavaScript 失效时，桌面筛选目录保持展开，8 张卡片、全部正文和代码仍可读取。首页正文不增加工具入口，唯一入口是全站主导航中的“工具指南”。RSS 仍只包含 17 篇知识文章，不包含安装手册。
...
- `public/assets/hub.js` 只负责移动导航、知识库/作品筛选、工具组合筛选、代码复制、回到顶部、结果播报、外链 toast 与封面失败后备；
