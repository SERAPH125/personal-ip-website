...
- Content: `src/content/notes/*.md` 是 17 篇文章与 4 条视频卡的唯一内容源；`src/content/tools/*.md` 是 20 篇工具指南的唯一内容源；其中 12 篇知识文章组成两条学习路线；资源位于 `public/assets/covers/*` 与 `public/assets/tools/*`；Astro 构建静态 HTML，`public/assets/hub.js` 只负责渐进交互
...
- Launch policy: 公开页面不得出现空链接或未确认素材；43 个 canonical 公开页面提供 description、canonical、Open Graph、RSS discovery、JSON-LD、favicon，并由 `robots.txt` / `sitemap-index.xml` 索引；既有 `.html` URL 永久保留
