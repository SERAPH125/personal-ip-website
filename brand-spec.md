...
- Content: `src/content/notes/*.md` 是 18 篇文章与 4 条视频卡的唯一内容源；`src/content/tools/*.md` 是 20 篇工具指南的唯一内容源；其中 12 篇知识文章组成两条学习路线；资源位于 `public/assets/covers/*` 与 `public/assets/tools/*`；Astro 构建静态 HTML，`public/assets/hub.js` 只负责渐进交互
...
- Tools IA (locked): **grouped_tool_directory + static_guides** — `tools.html` 桌面端使用左侧粘性工具目录，按语言模型与客户端 / 图像生成工具 / AI 编程工具 / 视频生成与剪辑分组列出 20 个名称，并锚点定位右侧同类卡片；卡片使用 `toolType`、`difficulty` 与费用摘要区分模型服务、客户端、编辑器、插件、工作流、运行时和命令行工具；820px 以下目录折叠，640px 以下卡片单列；20 篇详情继续使用 `tools/<slug>.html`，首页正文不提供工具入口，只通过全局导航进入
- Notes IA (locked): **series_filter + list_detail** — `notes.html` + 18 篇详情；五个筛选为 `all / agent / observe / aivideo / industry`；18 篇阶段不显示搜索，约 30 篇或有明确需求时再评估 Pagefind；详情采用居中的 `42rem` 阅读列、白色阅读纸面、自动目录与 Markdown 富文本节奏
...
- Launch policy: 公开页面不得出现空链接或未确认素材；44 个 canonical 公开页面提供 description、canonical、Open Graph、RSS discovery、JSON-LD、favicon，并由 `robots.txt` / `sitemap-index.xml` 索引；既有 `.html` URL 永久保留
