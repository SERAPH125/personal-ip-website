...
- Tools IA (locked): **sidebar_catalog_filter + static_guides** — `tools.html` 桌面端使用左侧粘性目录承载类别 / 平台 / 使用方式组合筛选，右侧工具卡两列；820px 以下目录折叠，640px 以下卡片单列；8 篇详情继续使用 `tools/<slug>.html`，首页正文不提供工具入口，只通过全局导航进入
