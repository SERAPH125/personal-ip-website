...
- Content: `src/content/notes/*.md` 是 17 篇文章与 4 条视频卡的唯一内容源；其中 12 篇组成两条学习路线；封面位于 `public/assets/covers/*`；Astro 构建静态 HTML，`public/assets/hub.js` 只负责交互
- Primary nav (locked): **home / works / learn / notes / about** — `首页 / 作品 / 学习路线 / 知识库 / 关于`
...
- Launch policy: 公开页面不得出现空链接或未确认素材；22 个 canonical 公开页面提供 description、canonical、Open Graph、RSS discovery、JSON-LD、favicon，并由 `robots.txt` / `sitemap-index.xml` 索引；既有 `.html` URL 永久保留
